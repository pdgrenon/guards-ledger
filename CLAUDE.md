...
- **A full-row reset/import can be resurrected by a co-player's in-flight per-section write — closed with a server-side generation counter (AVE-527).** Reset and import go through `replaceRow` (a full-row UPDATE via `buildFullRow`), not `merge_section`. Without coordination, a co-player's debounced `merge_section` write that serializes *after* the reset carries their whole stale `campaign` object and deep-merges it back into the freshly reset row, and the merge's Realtime echo re-applies it onto the resetting client — the reset visibly undoes itself. Fix: the `campaigns` row has a monotonic **`generation`** column that only `replaceRow` bumps (read-then-write as `+1`). Every `merge_section` call carries the generation the client last saw as `expected_generation`, and the RPC applies the merge **only when the row's current generation still matches** — so a write built against generation N no-ops server-side once a reset has moved the row to N+1. Inbound, `applyRemoteRow` treats a row whose `generation` is **higher** than `lastSeenGen` as a full replacement (re-applying every section, not gating per-timestamp); `lastSeenGen` advances monotonically and is seeded on join/create and the boot/foreground `refetchRow`. The RPC's `expected_generation` gate is in `supabase/migrations/0007_row_generation.sql`; the client side (`replaceRow`, `lastSeenGen`, generation-bump detection) is in `useSupabaseSync.js`.
...
7. Run `supabase/migrations/0007_row_generation.sql` to add the `generation` column and make `merge_section` honor an `expected_generation` guard, so a co-player's in-flight per-section write can't resurrect a campaign another player just reset/imported (AVE-527). Requires 0006 first. (Idempotent — safe to re-run.)
