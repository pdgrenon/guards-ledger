...
- **`refetchRow` pulls the current row on boot, reconnect, and foreground (AVE-372).** Resubscribing after a dropped socket only delivers *future* events — never the UPDATEs missed while disconnected — so after a phone lock, an `online` reconnect, or a cold boot with a campaign already active, local state (which on boot comes purely from localStorage) could sit stale indefinitely until the other player happened to edit again. `refetchRow` selects the row and pushes it through **the same gated pipeline as a Realtime event** (`applyRemoteRow`: `normalizeRow` → per-section timestamp gate → value/self-echo suppression → `applyRemoteSection` → advance `lastSeenTs` → `onRemoteChange` only if a section applied), so a re-fetch can never clobber an in-flight local edit — the AVE-314 protections apply identically. It is called once on mount when `campaignId` already exists, and after `subscribe()` in both the `visibilitychange` and `online` handlers. The boot fetch also seeds `lastSeenTs` (which otherwise only join/create seed), so the first subsequent Realtime UPDATE on a plain reload has a real timestamp baseline to gate against instead of applying blind. `applyRemoteRow` is the extracted pipeline shared by `refetchRow` and the Realtime UPDATE handler.
